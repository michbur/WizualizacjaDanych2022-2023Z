...
group_by(dat, month, year, voivodeship) %>% 
    ...
    ggplot(aes(x = year, y = as.factor(month), fill = total, label = total)) +
    geom_tile(color = "navyblue") +
    geom_text(color = "red") +
...
group_by(dat, age, sex) %>% 
    summarise(total = sum(count)) %>% 
    ungroup() %>% 
    mutate(age_bin = cut(age, breaks = round(quantile(age, probs = seq(0, 1, 0.2)), 0), include.lowest = TRUE, ordered_result = TRUE)) %>% 
